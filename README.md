...
Este projeto foi desenvolvido como requisito avaliativo para a disciplina de **Compiladores** da **Escola Politécnica da Universidade de Pernambuco (POLI/UPE)**.

---

## 🚀 Filosofia do Projeto

Alinhado com a premissa prática da disciplina de que **Originalidade + Utilidade > Complexidade**, o `WorldCupCompiler` abstrai regulamentos esportivos internacionais complexos em uma sintaxe limpa, legível e altamente restritiva. 

O coração do projeto reside em seu **Analisador Semântico**, que atua como um auditor oficial das regras de sorteio e integridade da FIFA, impedindo inconsistências lógicas antes de computar os resultados.
...
* `src/`: Contém os componentes fundamentais do front-end da linguagem.
    ...
    * `semantic.py`: Analisador semântico estruturado sobre o percorrimento da AST (Abstract Syntax Tree) com validação de escopo e contexto.
    * `interpreter.py`: Motor de processamento, cálculo de pontuações padrão FIFA e geração da classificação final.
...
1.  **Regra de Sorteio (Restrição Geográfica):** Impede a presença de mais de uma seleção da mesma confederação continental no mesmo grupo (excetuando a Europa, que aceita até duas por regulamento).
